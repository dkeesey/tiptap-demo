#!/bin/bash
# Railway deployment script for TipTap Collaborative Editor (Updated for Railway CLI v4+)
...
PROJECT_EXISTS=$(railway list | grep "$PROJECT_NAME" || echo "not found")

if [[ "$PROJECT_EXISTS" == *"not found"* ]]; then
    echo -e "${YELLOW}Creating new Railway project: $PROJECT_NAME...${RESET}"
    railway init --name "$PROJECT_NAME"
else
    echo -e "${GREEN}Using existing project: $PROJECT_NAME...${RESET}"
    # For existing projects, we need to link to it
    railway link
fi
...
# Create websocket service (updated to use 'new')
echo -e "${GREEN}Creating WebSocket service...${RESET}"
# The service creation command is now different
railway service new --name websocket-server
...
# Adjusted for current Railway CLI syntax
WS_URL=$(railway status | grep -o 'https://[^ ]*' | head -1 | sed 's/https:/wss:/')

if [ -z "$WS_URL" ]; then
    echo -e "${RED}Failed to get WebSocket URL. Using default.${RESET}"
    WS_URL="wss://websocket-server-production.up.railway.app"
    
    # Alternative approach to get URL
    echo -e "${YELLOW}Trying alternative approach to get WebSocket URL...${RESET}"
    WS_URL=$(railway show | grep -o 'https://[^ ]*' | head -1 | sed 's/https:/wss:/')
    
    if [ -z "$WS_URL" ]; then
        echo -e "${RED}Alternative approach also failed. Using default URL.${RESET}"
        WS_URL="wss://websocket-server-production.up.railway.app"
    fi
fi
...
railway service new --name frontend
...
FRONTEND_URL=$(railway status | grep -o 'https://[^ ]*' | head -1)

if [ -z "$FRONTEND_URL" ]; then
    echo -e "${RED}Failed to get frontend URL. Trying alternative approach...${RESET}"
    FRONTEND_URL=$(railway show | grep -o 'https://[^ ]*' | head -1)
    
    if [ -z "$FRONTEND_URL" ]; then
        echo -e "${RED}Alternative approach also failed. Cannot determine frontend URL.${RESET}"
    fi
else
