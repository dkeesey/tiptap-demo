import React, { useState } from 'react';
import { useRailwayCollaboration } from '../context/RailwayCollaborationContext';
import { ConnectionStatus as ConnectionState } from '../context/RailwayCollaborationContext';

interface ConnectionStatusProps {
  showDetails?: boolean;
  className?: string;
}

const ConnectionStatusIndicator: React.FC<ConnectionStatusProps> = ({ 
  showDetails = true,
  className = ''
}) => {
  const { connectionStatus, connectionDetails, forceSync } = useRailwayCollaboration();
  const [expanded, setExpanded] = useState<boolean>(false);
  
  // Status badge styles
  const getBadgeColor = () => {
    switch (connectionStatus) {
      case 'connected': return 'bg-green-500';
      case 'connecting': return 'bg-blue-500';
      case 'reconnecting': return 'bg-yellow-500';
      case 'disconnected': return 'bg-gray-500';
      case 'error': return 'bg-red-500';
      default: return 'bg-gray-500';
    }
  };

  const toggleExpanded = () => {
    setExpanded(prev => !prev);
  };
  
  const getDetailsText = () => {
    if (connectionStatus === 'error' || connectionStatus === 'disconnected') {
      return `Attempting to connect to: ${connectionDetails.url || 'unknown'}`;
    }
    return '';
  };
  
  return (
    <div className={`fixed bottom-4 right-4 flex flex-col items-end z-50 ${className}`}>
      <div 
        className={`${getBadgeColor()} text-white px-3 py-1 rounded-full text-sm flex items-center cursor-pointer`}
        onClick={toggleExpanded}
      >
        <div className={`w-2 h-2 rounded-full mr-2 ${connectionStatus === 'connected' ? 'animate-pulse' : ''} bg-white`}></div>
        {connectionStatus.charAt(0).toUpperCase() + connectionStatus.slice(1)}
      </div>
      
      {showDetails && (expanded || getDetailsText()) && (
        <div className="mt-2 bg-white bg-opacity-90 text-xs text-gray-800 p-2 rounded shadow-md max-w-[250px]">
          {getDetailsText() || (
            connectionStatus === 'connected' ? 
              'Connected to collaboration server' : 
              'Click for connection details'
          )}
          
          {expanded && (
            <div className="mt-2 pt-2 border-t border-gray-200">
              <button 
                className="bg-blue-500 text-white text-xs px-2 py-1 rounded"
                onClick={() => forceSync()}
              >
                Force Sync
              </button>
              
              <button 
                className="bg-gray-500 text-white text-xs px-2 py-1 rounded ml-2"
                onClick={() => {
                  console.log('[Connection Details]', connectionDetails);
                }}
              >
                Debug Info
              </button>
            </div>
          )}
        </div>
      )}
    </div>
  );
};

export default ConnectionStatusIndicator;
