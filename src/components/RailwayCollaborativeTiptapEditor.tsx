...
import RailwayConnectionStatus from './Debug/RailwayConnectionStatus'
...
interface RailwayCollaborativeTiptapEditorProps {
...
const RailwayCollaborativeTiptapEditor = ({ 
  onChange, 
  aiEnabled = true,
  room = 'default-room'
}: RailwayCollaborativeTiptapEditorProps) => {
  console.log('[Railway Editor] Rendering RailwayCollaborativeTiptapEditor for room:', room);
  ...
  useEffect(() => {
    ...
    if (testUserId && testUserName && testUserColor) {
      console.log('[Railway Editor] Setting up test user:', { testUserId, testUserName, testUserColor });
  ...
  useEffect(() => {
    const observer = () => {
      console.log('[Railway Editor Y.Doc] Document fragment updated:', {
    ...
    console.log('[Railway Editor Y.Doc] Initial document state:', {
      fragmentLength: fragment.length, 
      isEmpty: fragment.length === 0,
      timestamp: new Date().toISOString(),
    });
  ...
  useEffect(() => {
    if (isConnected) {
      setStatusMessage('Connected to Railway WebSocket server')
    } else if (connectionStatus === 'connecting') {
      setStatusMessage('Connecting to Railway WebSocket server...')
    } else if (connectionStatus === 'reconnecting') {
      setStatusMessage('Reconnecting to Railway WebSocket server...')
    } else if (error) {
      setStatusMessage('Connection error - Changes are saved locally and will sync when reconnected')
    } else {
      setStatusMessage('Disconnected - Trying to reconnect...')
    }
  ...
  useEffect(() => {
    ...
    if (connectionStatus === 'disconnected' && !isConnected && !error) {
      reconnectTimer = setTimeout(() => {
        console.log('[Railway Editor] Auto-reconnecting...');
        reconnect();
      }, 5000);
    }
  ...
  }, [connectionStatus, isConnected, error, reconnect]);

  if (!editor) {
    return <div className="p-4 text-center text-gray-500">Loading Railway collaborative editor...</div>
  }

  return (
    <div className="railway-tiptap-editor" ref={editorRef}>
        ...
        <div className="flex items-center justify-between">
          ...
          {isConnected && provider?.awareness && (
            <div className="text-xs">
              {connectedUsers.size || 1} user{(connectedUsers.size || 1) !== 1 ? 's' : ''} connected
            </div>
          )}
        </div>
      </div>
      ...
      {/* Railway connection status with detailed info */}
...
export default RailwayCollaborativeTiptapEditor
